...
import { FileText, MonitorPlay, Presentation, Link as LinkIcon, FileArchive, Image as ImageIcon, File } from 'lucide-react';
...
export const ResourceBadge: React.FC<ResourceBadgeProps> = ({ resource, compact = false }) => {
  const getIcon = () => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    if (window.electronAPI && resource.url && !resource.url.startsWith('http')) {
      e.preventDefault();
      window.electronAPI.openPath(resource.url);
    }
  };
  ...
  return (
    <a
      ...
      onClick={handleClick}
      className={`
        flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg hover:border-brand-300 hover:shadow-sm transition-all group
